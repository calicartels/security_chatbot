...
from cve_fetch import fetch_nvd, fetch_osv, normalize_cves, fetch_nvd_by_id
from vector_db import VectorDB
from llm_query import embed_text, chat_response
from cve_processing import filter_and_rank_cves, parse_infrastructure
        ...
        with st.chat_message("assistant"):
            with st.spinner("Searching and analyzing..."):
                ...
                for comp in components:
                    name = comp.get("name")
                    if name:
                        component_terms.append(name)
                    version = comp.get("version")
                    if version:
                        component_terms.append(version)

                combined_query = prompt
                if component_terms:
                    combined_query = f"{prompt} {' '.join(component_terms)}".strip()

                raw_cves = st.session_state.vector_db.search_relevant(combined_query, k=15)

                mentioned_ids = {m.upper() for m in re.findall(r"cve-\d{4}-\d+", prompt, flags=re.IGNORECASE)}
                existing_ids = {cve.get("id", "").upper() for cve in raw_cves}

                for cve_id in mentioned_ids:
                    if cve_id in existing_ids:
                        continue

                    cve_obj = st.session_state.vector_db.get_cve_by_id(cve_id) if st.session_state.cves_loaded else None
                    if not cve_obj:
                        fetched = fetch_nvd_by_id(cve_id)
                        if fetched:
                            normalized = normalize_cves([fetched])
                            cve_obj = normalized[0] if normalized else None
                    if cve_obj:
                        raw_cves.append(cve_obj)
